package com.wtb.hook.core

import com.wtb.hook.callMethod
import com.wtb.hook.log
import com.wtb.hook.tryCatch
    ...
    companion object {
        ...
        fun findClass(className: String?, targetCls: ClassLoader?=null): Class<*>? {
            ...
            return CachePool.getClass(className) {
                ...
                val targetClass = XposedHelpers.findClass(className, tempCls)
                log("find targetClass=$targetClass")
                targetClass
            }
        }
    }
    ...
    private fun hookWrapper(targetCls: Class<*>?, methodName: String?, vararg paramsTypesAndCallback: Any?) {
        ...
        tryCatch {
            val hookInstance = if(methodName.isNullOrEmpty()) {
                ...
                val hookResult = XposedHelpers.findAndHookMethod(
                    targetCls,
                    methodName,
                    *paramsTypesAndCallback
                )
                log("hook statue=${hookResult.hookedMethod}")
                hookResult
            }
        }
    }
